...
<p><strong>面向长期 AI 编程项目的 Agent Skill：维护单一事实来源，对账文档与代码，追踪决策，归档过期方案，并在上下文中断后安全接续。</strong></p>
...
和 AI 做项目做到第 N 周，通常不会先遇到代码 Bug，而是先遇到：
...
**Project Canon** 是一个面向长期 AI 编程项目的治理 Skill。它帮助 Agent 找到项目已有的权威入口，区分候选、决定、正式变更、实现状态与验证证据，并在上下文中断后安全接续。
...
| 使用前 | 使用后 |
| --- | --- |
| 多份“最终方案”互相争夺权威 | 当前方案唯一，决策、变更、证据相互链接 |
| AI 根据聊天记忆继续 | 每次重新读取项目真实状态 |
| 过期文档反复冒充真相 | 已完成材料归档并建立来源映射 |
| “已实现”被当成“已验证” | 每种状态有独立位置与证据 |
| 每次新会话重复旧决策 | 已有决策被保留，可随时审计 |
